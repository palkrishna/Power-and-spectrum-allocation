...
def calpow(epsi,sig2,Pc_max,Pd_max,alpha_k,alpha_mk,epsi_k,epsi_mk,h_k,h_mk,p0,gamma0) :
  den0 = alpha_mk*(1-epsi_mk**2)*(1/p0-1)*epsi_k**2*abs(h_k) - (1-epsi_k**2)*alpha_mk*epsi_mk**2*abs(h_mk)
  Pc0 = (1-epsi_k**2)*sig2/den0
  Pd0 = Pc0*gamma0*alpha_mk*(1-epsi_mk**2)*(1-p0)/(alpha_k*(1-epsi_k**2)*p0)
  if Pd_max <= Pd0 :
    B = Pd_max*alpha_k*(1-epsi_k**2)
    C = sig2+Pc_max*epsi_mk**2*alpha_mk*abs(h_mk)
    D = Pc_max*alpha_mk*(1-epsi_mk**2)
    
    tmp = 1/(1-p0)*exp(epsi_k**2*abs(h_k)/(1-epsi_k**2))
    if exp(C*gamma0/B)*(1+D/B*gamma0) - tmp > 0 :
      ...
      while(abs(P_right-P_left)>epsi) :
            ...
            C = sig2+P_mid*epsi_mk**2*alpha_mk*abs(h_mk)
      ...
      Pc_opt = P_mid
        ...
        P_left = 0
        P_right = Pd_max
        C = sig2+Pc_max*alpha_mk*epsi_mk**2*abs(h_mk)
        D = Pc_max*alpha_mk*(1-epsi_mk**2)
    ...
    num = (epsi_mk**2*abs(h_mk))/(1-epsi_mk**2)
    
    A = Pd_max*alpha_k*epsi_k**2*abs(h_k)
    B = Pd_max*alpha_k*(1-epsi_k**2)
    D = Pc_max*alpha_mk*(1-epsi_mk**2)
    ...
    if num-(den1+den2)-log10(p0) > 0 :
        ...
        while(abs(P_right-P_left)>epsi) :
            P_mid = (P_left+P_right)/2
            A = P_mid*alpha_k*epsi_k**2*abs(h_k)
            B = P_mid*alpha_k*(1-epsi_k**2)
